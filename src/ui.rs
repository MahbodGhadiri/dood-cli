...
pub async fn interactive_chat(username: &str) -> Result<()> {
    ...
    println!("{}", "─".repeat(60).bright_black());
    println!("{}", "Type your message and press Enter. Type '/quit' to exit.".bright_black());
    println!();
    ...
    for msg in messages.iter().rev() {
        if msg.is_outgoing {
            ...
            println!("{} {}", format!("{}:", username).bold().green(), msg.content);
        }
    }
...
pub fn display_account_info() -> Result<()> {
    ...
    println!("{} {}", "Identity Key:".bold(), truncate(&identity_pub_b64, 50).bright_black());
    println!();
